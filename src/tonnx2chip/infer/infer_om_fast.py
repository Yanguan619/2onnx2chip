...
def compute_3d_rope(
    ...
    if mm_token_type_ids is not None and image_grid_thw is not None:
        ...
        # 使用torch找到类型变化的位置
        # 计算相邻token类型的变化
        diff = torch.cat([torch.tensor([1], device=device), token_types[1:] != token_types[:-1]])
        change_indices = torch.where(diff)[0]

        # 构建groups: (type, start, end)
        groups = []
...
def compute_3d_rope_numpy(
    seq_len, mm_token_type_ids=None, image_grid_thw=None, spatial_merge_size=2
):
    """Replicate Qwen3.5Model.get_rope_index for 3D M-RoPE.
...
class Qwen35OM:
    ...
    LINEAR_ATTN_LAYERS = tuple(i for i in range(24) if i not in [3, 7, 11, 15, 19, 23])
    ...
    def extract_states(self, prefill_outputs):
        ...
        """
        states = [[None, None, None] for _ in range(24)]
        for layer in range(24):
            base = 1 + layer * 2
            typ = "attn" if layer in self.FULL_ATTN_LAYERS else "linear"
    ...
    def generate(self, prompt, image_path=None, max_new_tokens=128):
        ...
        with tqdm(range(1), desc="Prefilling", unit="tok") as pbar:
            pos_3d = compute_3d_rope(seq_len, mm_token_type_ids, image_grid_thw)
        ...
        with tqdm(range(max_new_tokens), desc="Decoding", unit="tok") as pbar:
            for step in pbar:
                ...
                past_len = states[self.FULL_ATTN_LAYERS[0]][1].shape[2]
        ...
        for layer in self.FULL_ATTN_LAYERS[:3]:
            print(f"  past_{layer}_k: {states[layer][1].shape}", flush=True)
        for layer in self.LINEAR_ATTN_LAYERS[:3]:
            print(
...
def infer(
    ...
    max_new_tokens: int = TARGET_SEQ_LEN,
    device_id: int = 0,
):
