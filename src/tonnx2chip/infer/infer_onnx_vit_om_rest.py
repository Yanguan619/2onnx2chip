"""Hybrid Qwen3.5 pipeline: ONNX vision encoder + OM embed/prefill/decode.

This mirrors infer_qwen3p5_om.py but swaps the vision encoder (VIT) backend
from Ascend OM (ais_bench InferSession) to ONNX Runtime on CPU. The embedding,
prefill and decode stages still run on NPU via OM.

Motivation: isolate whether the VIT OM graph introduces precision issues.
Everything downstream (embed/prefill/decode OM) is unchanged, so any mismatch
with the pure-OM reference must originate in the vision encoder stage.

Differences from infer_qwen3p5_om.py:
  - `vit` is an onnxruntime.InferenceSession (CPU), not an OM InferSession.
  - Vision encode feeds a name-keyed numpy dict {"pixel_values": ...}, matching
    val_qwen3p5_onnx.py. Output is host numpy, identical shape/dtype to OM.
  - embed/prefill/decode are byte-for-byte the OM code paths.
"""
...
class Qwen35Hybrid:
    def __init__(
        ...
        # === CPU side: vision encoder (ONNX Runtime) ===
        print(f"\nLoading ONNX vision encoder: {vit_onnx_path}", flush=True)
        so = ort.SessionOptions()
        so.enable_cpu_mem_arena = False
        so.graph_optimization_level = ort.GraphOptimizationLevel.ORT_DISABLE_ALL
        so.intra_op_num_threads = 1
        self.vit = ort.InferenceSession(
            vit_onnx_path, providers=ONNX_PROVIDER, session_options=so
        )
        self.vit_input_names = [i.name for i in self.vit.get_inputs()]

        # === NPU side: embedding, prefill, decode (OM) ===
        print("\nLoading OM models (embed / prefill / decode)...", flush=True)
    ...
    def vision_encode(self, pixel_values):
        ...
        missing = [n for n in self.vit_input_names if n not in feeds]
        if missing:
            raise RuntimeError(
                f"ONNX vit graph expects inputs not provided: {missing}"
            )
        out = self.vit.run(None, feeds)
        return out[0]
    ...
    def generate(self, prompt, image_path=None, max_new_tokens=128):
        ...
        # === Step 1: Vision encoder (ONNX, CPU) ===
        if px is not None:
            with tqdm(range(1), desc="Running ONNX vision encoder..", unit="tok") as pbar:
...
@app.command()
def infer(
    vit_onnx_path: str = typer.Option(..., help="VIT ONNX path (used in place of OM vit)"),
    ...
    for f in [vit_onnx_path, embedding_path, decoder_prefill_path, decoder_decode_path]:
    ...
    print("════════" * 4)
    print(f"Hybrid ONNX-vit + OM-embed/prefill/decode ({len(tokens)} tok, {elapsed:.2f}s):")
    print(text)
    print("════════" * 4)
